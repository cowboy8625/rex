...
pub const VisibleEntities = struct {
    ...
    fn deinit(self: *VisibleEntities, allocator: std.mem.Allocator) void {
...
pub const Engine = struct {
    ...
    pub fn run(self: *Engine) !void {
        ...
        self.addSystem(.Update, visibilitySystem);
        ...
        while (true) {
            ...
            try renderer.setDrawColor(sdl3.pixels.Color{ .r = 0, .g = 0, .b = 0, .a = 255 });
            try renderer.clear();
    ...
    fn renderShapeSystem(self: *Engine, renderer: *const sdl3.render.Renderer) !void {
        // Get camera position (for now, assume just one camera)
